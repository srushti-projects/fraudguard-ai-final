...
export default function PostCard({ post }) {
  ...
  const [commentsCount, setCommentsCount] = useState(post.comments_count ?? post.comments.length);
  ...
  return (
    ...
    >
      ...
      <AnimatePresence>
        {showComments && <CommentSection postId={post.id} onCommentAdded={() => setCommentsCount((prev) => prev + 1)} />}
      </AnimatePresence>
    </motion.div>
  );
}
